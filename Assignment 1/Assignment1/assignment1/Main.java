...
public class Main {
	...
	boolean isValidCollectionInput(String collectionString) {
		...
		if (identifierInputArray.length == 0) {
			return true;
		}
		...
		for (String identifierInput : identifierInputArray) {
			if (!identifierInput.equals("")) {
				try {
					...
					// Cannot occur because collection size is checked in
					// isValidCollectionInput to be limited to 10 per
					// collection.
	...
	void processCollection(Collection collection1, Collection collection2) {
		...
		try {
			...
			// Cannot occur because collection size is checked in
			// isValidCollectionInput to be limited to 10 per collection.
			e.printStackTrace();
		}
